...
async def set_student_book(user_id: int, book_id: int):
    """Teacher action — move student to a different book (lesson resets to 1)."""
    await _pool.execute(
        "UPDATE users SET current_book=$2, current_lesson=1, state='idle' WHERE user_id=$1",
        user_id, book_id,
    )
    # Clear any active session for this student
    await _pool.execute("DELETE FROM sessions WHERE user_id=$1", user_id)


async def boost_student_rank(user_id: int):
    """Teacher action — mark current lesson's words as learned and advance lesson by 1."""
    from words import get_lesson_words, get_total_lessons
    row = await _pool.fetchrow("SELECT current_book, current_lesson FROM users WHERE user_id=$1", user_id)
    if not row:
        return
    book = int(row["current_book"] or 1)
    lesson = int(row["current_lesson"] or 1)
    total = get_total_lessons(book)

    # Mark all words of current lesson as learned
    lesson_words = get_lesson_words(lesson, book)
    week = lesson  # use lesson as week identifier
    for w in lesson_words:
        await _pool.execute("""
            INSERT INTO progress (user_id, word_id, status, week, book)
            VALUES ($1, $2, 'learned', $3, $4)
            ON CONFLICT (user_id, word_id) DO UPDATE SET status='learned'
        """, user_id, w["id"], week, book)

    # Advance lesson (stay in same book if at max)
    next_lesson = lesson + 1 if lesson < total else lesson
    await _pool.execute(
        "UPDATE users SET current_lesson=$2, state='idle' WHERE user_id=$1",
        user_id, next_lesson,
    )
    await _pool.execute("DELETE FROM sessions WHERE user_id=$1", user_id)
